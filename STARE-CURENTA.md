...
**Versiune sistem:** V57 (consolidat · marker SHA în `_system/VERSIUNI.md`)
...
3. **Curățare drift SPEC C05** (3 runde feedback extern G-06, toate verificate empiric înainte de aplicare): (a) DOVADA TRANSFORMĂRII rescrisă din probă de normalizare C04 (volum/subtotaluri/SUM-AGGREGATE/Refresh) în probă de dicționar; (b) mecanism dicționar Format-as-Table+Refresh → **formule dinamice UNIQUE/COUNTUNIQUE** (afirmația „apare automat la valoare nouă" devine corectă tehnic); (c) alertă C06 → alertă de dicționar; (d) suma 7.986.019,38 lei scoasă (R-V02.15) în Studiu+Video; „Media ~133" scos; (e) tag AGGREGATE (C01/C04) → COUNTUNIQUE/COUNTIF (header + side-nav); (f) nav Etapa 5 Power Query Refresh → Formule dinamice; (g) final check 8 + completion-title curățate de sumă/generic; (h) „TOP-3 97.5%" headline → „TOP-3 DOMINĂ" (cifra rămâne doar în pasul de calcul, ca martor); (i) `next-label` TREAPTA 1 → 2.
...
| **C05** | DICȚIONAR (ce reprezintă datele — inventar categorii) | **V57 LIVRABILĂ** model PREMIUM (hero cockpit DICȚIONAR + arc TU + before/after + outcomes) + G-06 + curățare drift SPEC cap-coadă (zero contaminare C04/C06/KPI; mecanism dinamic UNIQUE/COUNTUNIQUE) | versiune unica + assets/ + hero dedicat | ✓ ZERO DRIFT · GATE PASS |
